...
## 5. Box’s M Test (manual approach from lecture slides)
```{r}
...
# Correction factor C
C <- (1/(n1 - 1) + 1/(n2 - 1) - 1/(n1 + n2 - 2)) * (2 * p^2 + 3 * p - 1) / (6 * (p + 1) * (n1 + n2 - 2))
Chi2_stat <- M * (1 - C)
df_boxm <- p * (p + 1) / 2
pvalue_boxm <- 1 - pchisq(Chi2_stat, df = df_boxm)
Chi2_stat; pvalue_boxm
```
...
- Box's M test suggests it is **reasonable to assume equal covariance matrices**.
